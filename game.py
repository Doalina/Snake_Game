...
def initialize_pygame():
    ...
    icon = pygame.image.load(CAME_ICON)
    pygame.display.set_icon(icon)
    pygame.display.set_caption(CAME_TITLE)
...
def check_collisions(game_state):
    ...
    if (
            x < 0 or y < 0 or x >= SIZE_X or y >= SIZE_Y or len(game_state['snake']) > len(set(game_state['snake']))
    ):
        game_state['game_running'] = False
